      ...
      footer={
        <>
          <button onClick={save} disabled={saving || loading}
            style={{ flex: 1, background: '#0176D3', border: 'none', borderRadius: 10, color: '#fff', fontWeight: 700, height: 48, cursor: 'pointer', opacity: saving ? 0.6 : 1 }}>
            {saving ? 'Saving…' : 'Save Review'}
          </button>
          <button onClick={onClose} style={{ background: '#F3F2F2', border: '1px solid #DDDBDA', borderRadius: 10, color: '#181818', fontWeight: 600, height: 48, padding: '0 24px', cursor: 'pointer' }}>Close</button>
        </>
      }
      headerExtra={
