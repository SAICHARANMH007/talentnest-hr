...
export default function ReferEarnModal({ job, onClose }) {
  ...
  // Auto-generate when modal opens if user is logged in
  useEffect(() => {
  ...
  const generate = async () => {
    ...
    try {
      ...
      const generatedLink = r?.link || r?.data?.link || '';
  ...
  const box     = { background: '#fff', borderRadius: 20, width: '100%', maxWidth: 460, padding: 28, boxShadow: '0 20px 60px rgba(0,0,0,0.2)', position: 'relative' };
  const btn     = (bg, color = '#fff') => ({ background: bg, color, border: 'none', borderRadius: 10, padding: '11px 18px', fontSize: 13, fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 6 });

  return (
    <div style={overlay} onClick={e => e.target === e.currentTarget && onClose()}>
      <div style={box}>
        {/* Close */}
        <button onClick={onClose} style={{ position: 'absolute', top: 14, right: 14, background: '#F1F5F9', border: 'none', borderRadius: 8, width: 32, height: 32, cursor: 'pointer', fontSize: 16, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>✕</button>

        {/* Header */}
        <div style={{ textAlign: 'center', marginBottom: 20 }}>
          <div style={{ fontSize: 40, marginBottom: 8 }}>🤝</div>
          <h2 style={{ margin: 0, fontSize: 18, fontWeight: 800, color: '#0A1628' }}>Refer & Earn</h2>
          <p style={{ margin: '6px 0 0', fontSize: 13, color: '#6B7280' }}>Share this job with your network</p>
        </div>
        ...
        {rewardAmt != null && rewardAmt > 0 && (
          <div style={{ background: 'linear-gradient(135deg,#FEF3C7,#FDE68A)', border: '1px solid #F59E0B', borderRadius: 12, padding: '12px 16px', marginBottom: 16, display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <div>
            ...
            <div style={{ display: 'flex', gap: 6, marginBottom: 14 }}>
              <input readOnly value={link}
                style={{ flex: 1, padding: '10px 12px', borderRadius: 8, border: '1.5px solid #CBD5E1', fontSize: 11, background: '#F8FAFC', color: '#374151', outline: 'none', minWidth: 0, wordBreak: 'break-all' }}
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 10 }}>
              ...
              <button onClick={shareEmail} style={btn('#374151')}>
                ✉️ Email
              </button>
            </div>
