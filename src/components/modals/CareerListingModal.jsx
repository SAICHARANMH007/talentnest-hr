      ...
      <div style={{ background:'#fff', borderRadius:20, width:'100%', maxWidth:720, maxHeight:'92vh', display:'flex', flexDirection:'column', boxShadow:'0 24px 64px rgba(0,0,0,0.22)', overflow:'hidden' }}>
          ...
          <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start' }}>
            <div style={{ flex:1, minWidth:0 }}>
              ...
              {isSuperAdminSelect ? (
                ...
                }} style={{ background:'rgba(255,255,255,0.15)', color:'#fff', border:'1px solid rgba(255,255,255,0.3)', borderRadius:8, padding:'6px 12px', fontSize:14, fontWeight:700, outline:'none', cursor:'pointer', marginTop:4, maxWidth:'100%' }}>
                  {orgsList.map(o => <option key={o.id || o._id} value={o.id || o._id} style={{ color:'#181818' }}>{o.name}</option>)}
                </select>
              ) : (
                <h2 style={{ color:'#fff', margin:'4px 0 0', fontSize:18, fontWeight:800 }}>🌐 {activeOrg?.name}</h2>
              )}
              <p style={{ color:'rgba(255,255,255,0.65)', fontSize:12, margin:'6px 0 0' }}>Select which jobs appear on your public career page</p>
            </div>
        ...
        <div style={{ padding:'16px 20px', background:'#F8FAFC', borderBottom:'1px solid #E2E8F0', flexShrink:0 }}>
          ...
          <div style={{ display:'flex', gap:8, alignItems:'center', flexWrap:'wrap' }}>
            <div style={{ flex:1, background:'#fff', border:'1px solid #E2E8F0', borderRadius:8, padding:'8px 12px', fontSize:13, color:'#0176D3', fontWeight:600, wordBreak:'break-all', minWidth:180 }}>
              {careerUrl || (orgSlug ? '...' : '⚠️ Org slug not set — contact super admin')}
            </div>
            {careerUrl && (
              <>
          ...
          {careerUrl && (
            <details style={{ marginTop:12 }}>
              <summary style={{ fontSize:12, color:'#64748B', cursor:'pointer', fontWeight:700, userSelect:'none', padding:'6px 0' }}>
                📦 Embed on your website (iframe code) — click to expand
              </summary>
              <div style={{ marginTop:8, background:'#0f172a', borderRadius:10, padding:'14px 16px', position:'relative' }}>
                <pre style={{ color:'#7DD3FC', fontSize:11, margin:0, whiteSpace:'pre-wrap', wordBreak:'break-all', lineHeight:1.7 }}>{embedCode}</pre>
                <button onClick={copyEmbed}
                  style={{ position:'absolute', top:8, right:8, background:copiedEmbed?'#059669':'rgba(255,255,255,0.12)', border:'none', borderRadius:6, color:'#fff', fontSize:11, padding:'4px 10px', cursor:'pointer', fontWeight:700, transition:'background 0.2s' }}>
                  {copiedEmbed ? '✅ Copied' : 'Copy'}
                </button>
              </div>
              <p style={{ fontSize:11, color:'#94A3B8', margin:'8px 0 0', lineHeight:1.6 }}>
                Paste this snippet anywhere on your website. The job board renders inside your page and updates automatically when you toggle jobs here.
              </p>
            </details>
          )}
        </div>
        ...
        <div style={{ padding:'12px 20px', borderBottom:'1px solid #E2E8F0', display:'flex', gap:10, alignItems:'center', flexShrink:0, flexWrap:'wrap' }}>
          ...
          <span style={{ fontSize:13, color:'#64748B', whiteSpace:'nowrap' }}>
            <b style={{ color:'#0176D3' }}>{selected.size}</b> of {jobs.length} listed
          </span>
        ...
        {/* Footer */}
        <div style={{ padding:'14px 20px', borderTop:'1px solid #E2E8F0', display:'flex', gap:10, justifyContent:'space-between', alignItems:'center', background:'#FAFAFA', flexShrink:0, flexWrap:'wrap' }}>
          <span style={{ fontSize:12, color:'#94A3B8' }}>
            {careerUrl ? <>Career page: <a href={careerUrl} target="_blank" rel="noopener noreferrer" style={{ color:'#0176D3', fontWeight:600 }}>{careerUrl}</a></> : 'Career URL pending slug setup'}
          </span>
          <div style={{ display:'flex', gap:10 }}>
            <button onClick={onClose} style={{ padding:'10px 20px', border:'1px solid #E2E8F0', background:'#fff', borderRadius:10, color:'#374151', fontWeight:600, fontSize:13, cursor:'pointer' }}>Cancel</button>
