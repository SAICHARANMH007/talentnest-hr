    ...
    <div style={{
      ...
      <div style={{ flex:1, minWidth:0 }}>
        <div style={{ display:'flex', alignItems:'center', gap:8, flexWrap:'wrap' }}>
          ...
          {isCurrent && (
            <span style={{ fontSize:10, fontWeight:800, color:'#059669', background:'rgba(5,150,105,0.1)', padding:'2px 8px', borderRadius:20 }}>
              ● ACTIVE
            </span>
          )}
        </div>
        ...
        <div style={{ display:'flex', gap:12, flexWrap:'wrap', marginTop:3 }}>
          {entry.recruiterEmail && (
            <a href={`mailto:${entry.recruiterEmail}`} style={{ fontSize:11, color:'#0176D3', textDecoration:'none' }}>
              ✉️ {entry.recruiterEmail}
            </a>
          )}
          {entry.recruiterPhone && (
            <a href={`tel:${entry.recruiterPhone}`} style={{ fontSize:11, color:'#059669', textDecoration:'none' }}>
              📞 {entry.recruiterPhone}
            </a>
          )}
        </div>
        ...
        <div style={{ fontSize:11, color:'#94A3B8', marginTop:3 }}>
          Assigned {fmt(entry.assignedAt)}
          {entry.removedAt && ` → Removed ${fmt(entry.removedAt)}`}
          {entry.assignedByName && ` · by ${entry.assignedByName}`}
        </div>
      </div>
    </div>
  );
