...
export default function CompanyDriveDetail() {
  ...
  // Live updates — silently re-sync when a student registers or their status/exam changes
  usePlatformEvents({
    ...
    <div>
      ...
      {loading ? (
        ...
        <>
          ...
          <div style={card}>
            {regs.length === 0 ? (
              <div style={{ color: '#706E6B', padding: 40, textAlign: 'center', fontSize: 14 }}>No students registered for this drive yet.</div>
            ) : regs.map(r => {
              ...
              return (
                <div key={r.candidateId} style={{ padding: '12px 0', borderBottom: '1px solid #F1F5F9', display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
                    ...
                    <span style={{ fontSize: 12, fontWeight: 700, padding: '4px 10px', borderRadius: 8, background: rc.bg, color: rc.color, textTransform: 'capitalize' }}>{r.status}</span>
                  </div>
                </div>
              );
            })}
