      ...
      {detailUser && (
        ...
        >
          ...
          <div style={{ marginTop: 24 }}>
            <h4 style={{ fontSize: 13, fontWeight: 800, color: '#014486', marginBottom: 12, textTransform: 'uppercase', letterSpacing: 0.5 }}>Application Pipeline</h4>
            <CandidatePipelinePanel 
              candidateEmail={detailUser.email} 
              candidateId={detailUser.id || detailUser._id} 
              onToast={setToast} 
            />
          </div>
        </Modal>
              ...
              <div key={u.id} style={{ ...card, border: `1px solid ${isSelected ? 'rgba(1,118,211,0.4)' : '#E8ECF0'}`, background: isSelected ? 'rgba(1,118,211,0.03)' : '#fff', transition: 'border-color 0.15s, background 0.15s', padding: '14px 16px', overflow: 'hidden' }}>
                ...
                {isCandidates && (u.experience || u.currentCTC || u.client || u.ta || u.jobRole || certArr.length > 0) && (
                  <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginTop: 10 }}>
                    ...
                    {(u.organisation || u.orgName || 'TalentNest HR') && <span style={{ background: '#F8FAFC', color: '#0176D3', fontSize: 11, padding: '3px 9px', borderRadius: 20, border: '1px solid rgba(1,118,211,0.2)', whiteSpace: 'nowrap' }}>🏢 {u.organisation || u.orgName || 'TalentNest HR'}</span>}
