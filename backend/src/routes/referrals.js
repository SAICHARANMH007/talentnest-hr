...
router.get('/', ...guard, allowRoles('admin', 'super_admin', 'recruiter'), asyncHandler(async (req, res) => {
  const refs = await Referral.find({ tenantId: req.tenantId })
...
router.post('/generate', ...guard, allowRoles('admin', 'super_admin', 'recruiter'), asyncHandler(async (req, res) => {
  ...
  const job = await Job.findOne({ _id: jobId, tenantId: req.tenantId, deletedAt: null }).lean();
...
router.get('/stats', ...guard, allowRoles('admin', 'super_admin'), asyncHandler(async (req, res) => {
  const [total, applied, hired, paid] = await Promise.all([
    ...
    Referral.countDocuments({ tenantId: req.tenantId, status: 'applied' }),
    Referral.countDocuments({ tenantId: req.tenantId, status: 'hired' }),
    Referral.countDocuments({ tenantId: req.tenantId, rewardPaid: true }),
  ]);
  const rewardSum = await Referral.aggregate([
    { $match: { tenantId: req.tenantId, status: 'hired', rewardPaid: false, rewardAmount: { $gt: 0 } } },
